...
export default function AdminDashboard() {
  ...
  // Wait, let's look at SVR Admin Client CRUD operations:
  // Add Client
  ...
  const fetchProgressDetails = async (projId) => {
    try {
      // Fetch public details (milestones)
      const res = await api.get(`/api/projects/${projId}`);
      // For message chat history and images, we can fetch from client dashboard style or a new endpoint.
      // Wait, let's create a specialized helper on client.js or admin.js if needed.
      // But since we can query messages via SQLite inside backend, let's make sure we have client data.
      // Let's implement an endpoint `/api/admin/projects/:id/progress-details` in `admin.js` to load chat & images.
      // Let's query it. Let's handle errors gracefully.
      const adminDetails = await api.get(`/api/projects/${projId}`);
      // We will set images list from milestones or fetch from client dashboard if we linked it.
      // Wait! Let's check if the adminDetails response has reviews, milestones, etc. Yes.
      // To support images and messages, we can write a route for admin progress-details:
      api.get(`/api/admin/projects-progress/${projId}`).then(details => {
        ...
        // Fallback or log
        console.log("No messages/images admin route yet. Let's write it.");
      });
    } catch (e) {
      console.log(e);
    }
  ...
  const handleImageUpload = async (e) => {
    ...
    if (uploadFile) {
      ...
      // Try accepting direct URL if input is text, or prompt upload file
